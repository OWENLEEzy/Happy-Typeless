...
_NEGATIVE_WORDS = ["烦", "累", "崩溃", "糟糕", "失败", "terrible", "annoying"]
_POSITIVE_WORDS = ["好", "棒", "完成", "成功", "great", "done", "nice"]
...
def generate_mock_analysis(transcription: Transcription) -> AITranscriptionAnalysis:
    ...
    has_negative = any(w in content for w in _NEGATIVE_WORDS)
    has_positive = any(w in content for w in _POSITIVE_WORDS)

    sentiment_score = -0.4 if has_negative else (0.5 if has_positive else rng.uniform(-0.1, 0.3))
    sentiment_label = (
        "negative"
    ...
    return AITranscriptionAnalysis(
        ...
        emotion=EmotionAnalysis(
            primary="anger" if has_negative else ("joy" if has_positive else "neutral"),
            intensity=round(rng.uniform(0.2, 0.6), 2),
        ),
        ...
        mental_health=MentalHealthIndicators(
            stress_level=round(rng.uniform(0.2, 0.6), 2),
            burnout_risk="low",
        ),
